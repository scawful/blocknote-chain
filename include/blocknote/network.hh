...
class RequestMessage {
 public:
  ...
  static bool parse(std::istream &stream, std::string &method,
    ...
    for (std::size_t c = 0; c < line.size(); ++c) {
      if (para_start_pos != std::string::npos) {
        if (para_end_pos == std::string::npos) {
          if (line[c] == ';') {
            result.emplace(line.substr(para_start_pos, c - para_start_pos),
                           std::string());
...
class ServerBase {
 ...
 public:
  class Response : public std::enable_shared_from_this<Response>,
   ...
   public:
    ...
    /// message
    void send(const std::function<void(const error_code &)> &callback =
                  nullptr) noexcept {
      session->connection->set_timeout(timeout_content);
      auto self =
          this->shared_from_this();  // Keep Response instance alive through the
                                     // following async_write
      asio::async_write(
    ...
    /// and empty content
    void write(
        enum status_code status_code = status_code::success_ok,
        const CaseInsensitiveMultimap &header = CaseInsensitiveMultimap()) {
      *this << "HTTP/1.1 " << network::status_code(status_code) << "\r\n";
      write_header(header, 0);
    }

    /// Convenience function for writing status line, header fields, and content
    void write(
        enum status_code status_code, const std::string &content,
        const CaseInsensitiveMultimap &header = CaseInsensitiveMultimap()) {
      *this << "HTTP/1.1 " << network::status_code(status_code) << "\r\n";
    ...
    /// Convenience function for writing status line, header fields, and content
    void write(
        enum status_code status_code, std::istream &content,
        const CaseInsensitiveMultimap &header = CaseInsensitiveMultimap()) {
      *this << "HTTP/1.1 " << network::status_code(status_code) << "\r\n";
    ...
    void write(
        ...
        const CaseInsensitiveMultimap &header = CaseInsensitiveMultimap()) {
      write(status_code::success_ok, content, header);
    }
    ...
    void write(const CaseInsensitiveMultimap &header) {
      write(status_code::success_ok, std::string(), header);
    }

    /// If true, force server to close the connection after the response have
    /// been sent.
    ///
  ...
  class Content : public std::istream {
   ...
   private:
    asio::streambuf &streambuf;
    explicit(false) Content(asio::streambuf &streambuf) noexcept
  ...
  class Request {
   ...
   public:
    ...
    CaseInsensitiveMultimap parse_query_string() noexcept {
      return network::QueryString::parse(query_string);
    }
  };
   ...
   public:
    ...
    void close() noexcept {
      ...
      std::unique_lock<std::mutex> lock(
          socket_close_mutex);  // The following operations seems to be needed
                                // to run sequentially
    ...
    Session(std::size_t max_request_streambuf_size,
      ...
      try {
        auto remote_endpoint =
            this->connection->socket->lowest_layer().remote_endpoint();
        request = std::shared_ptr<Request>(new Request(
            max_request_streambuf_size, remote_endpoint.address().to_string(),
            remote_endpoint.port()));
      } catch (...) {
        request =
            std::shared_ptr<Request>(new Request(max_request_streambuf_size));
      }
    }
  ...
  class regex_orderable : public regex::regex {
   ...
   public:
    regex_orderable(const char *regex_cstr)
        : regex::regex(regex_cstr), str(regex_cstr) {}
    regex_orderable(std::string regex_str)
        : regex::regex(regex_str), str(std::move(regex_str)) {}
 ...
 public:
  ...
  /// If you have your own asio::io_service, store its pointer here before
  /// running start().
        ...
        *session->connection->socket, session->request->streambuf, "\r\n\r\n",
        [this, session](const error_code &ec, std::size_t bytes_transferred) {
          ...
          if ((!ec || ec == asio::error::not_found) &&
            ...
            response->write(status_code::client_error_payload_too_large);
            response->send();
            if (this->on_error)
              this->on_error(session->request, make_error_code::make_error_code(
                                                   errc::message_size));
            return;
          }
          if (!ec) {
                    ...
                    [this, session](const error_code &ec,
                      ...
                      if (!ec) {
                        if (session->request->streambuf.size() ==
                          ...
                          response->write(
                              status_code::client_error_payload_too_large);
                          response->send();
                          if (this->on_error)
  ...
  void find_resource(const std::shared_ptr<Session> &session) {
    ...
    for (auto &regex_method : resource) {
      ...
      if (it != regex_method.second.end()) {
        regex::smatch sm_res;
        if (regex::regex_match(session->request->path, sm_res,
                               regex_method.first)) {
 ...
 protected:
  void accept() override {
    auto session = std::make_shared<Session>(config.max_request_streambuf_size,
                                             create_connection(*io_service));

    acceptor->async_accept(
        *session->connection->socket, [this, session](const error_code &ec) {
          auto lock = session->connection->handler_runner->continue_lock();
          if (!lock) return;
